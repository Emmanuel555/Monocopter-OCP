...
def transmitter_calibration():
    ...
    cmd = conPad*enable
    return (cmd, button0, button1, a0, a1, a2, a3)
...
def logging_config():
    ...
    # lg_stab.add_variable('ctrlINDI.rate_d[0]dd', 'float') # rad/s^2
    # lg_stab.add_variable('ctrlINDI.rate_d[1]', 'float')
    # lg_stab.add_variable('ctrlINDI.rate_d[2]', 'float')
...
def log_stab_callback(timestamp, data, logconf):
    ...
    omega_roll = data.get('ctrlINDI.Omega_f_p') # r rad/s
    omega_pitch = data.get('ctrlINDI.Omega_f_q') # p
    omega_yaw = data.get('ctrlINDI.Omega_f_r') # y
    # omega_roll_dot = data.get('ctrlINDI.rate_d[0]dd', 'float') # r rad/s^2
    # omega_pitch_dot = data.get('ctrlINDI.rate_d[1]', 'float') # p
    # omega_yaw_dot = data.get('ctrlINDI.rate_d[2]', 'float') # y
...
if __name__ == '__main__':
    ...
    with Swarm(uris, factory= CachedCfFactory(rw_cache='./cache')) as swarm:
        ...
        swarm.parallel(log_async, args_dict=seq_args_log)
        ...
        try:
            ...
            while True:
                ...
                if count % 10 == 0:
                    print('tx commands: ', cmd, button0, button1, a0, a1, a2, a3)
