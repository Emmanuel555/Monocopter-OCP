...
## generate the plot
fig = plt.figure(figsize=(40, 40)) # 40,12
...
fig.subplots_adjust(hspace=0.248, wspace=0.17, 
    ...
    for a in range(2): # 2 graphs per traj
        ...
        if a == 1:
            for m in range(2): # 0.5 INDI and NDI  
                ...
                graphs[a][t].set_xlabel('Time(s)',fontsize=53) #20
        ...
        # legend        
        graphs[a][t].legend(loc='upper right', fontsize=43) #20
        # axes
        graphs[a][t].tick_params(axis='both', labelsize=53) #20
        # add title
        graphs[a][t].set_title(traj_label+att_label, fontsize=53, fontweight='bold') #25
        graphs[a][t].grid(linewidth=4.0)



#plt.savefig(title+'.pdf')   
plt.savefig('short_wing_att.pdf', dpi=300, bbox_inches='tight')  
#plt.show()
