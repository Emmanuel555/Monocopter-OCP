...
for i in range(rows):
    for a in range(len(graphs[1])): # 2D plot for trajs
            ...
            for m in range((rows-1)*2): # 1.0 NMPC+INDI and DFBC+INDI
                if a < 2:
                    ...
                    if m == 0:
                        ...
                        method_label = '(PAYLOAD) NMPC+INDI'
                        style = 'dashed'
                    elif m == 3:
                        method_label = '(PAYLOAD) DFBC+INDI'
                        style = 'dashed'
                ...
                graphs[i][a].set_xlabel('Time(s)',fontsize=43) 
        ...
        if a<2:
            graphs[i][a].set_xlim(0,25)
        else:
            graphs[i][a].set_xlim(0,40)

        if i == 1 and a == 2:
            graphs[i][a].set_ylim(6,12)


        # legend        
        graphs[i][a].legend(loc='upper right', fontsize=38)
        # axes
        graphs[i][a].tick_params(axis='both', labelsize=43)
        # add title
        graphs[i][a].set_title(traj_label+att_label, fontsize=43, fontweight='bold')
        graphs[i][a].grid(linewidth=8.0)



#plt.savefig(title+'.pdf')   
plt.savefig('circle_att.pdf', dpi=300, bbox_inches='tight')  
#plt.show()
