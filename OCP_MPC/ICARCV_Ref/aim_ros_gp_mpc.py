...
from src.quad_mpc.aim_quad_3d_mpc import Quad3DMPC
import numpy as np
import std_msgs.msg
#from quadrotor_msgs.msg import ControlCommand
from ros_gp_mpc.msg import ControlCommand
import rospy
import os
...
def custom_quad_param_loader(quad_name):
    ...
    this_path = os.path.dirname(os.path.realpath(__file__)) # directory -> data_driven_mpc
    params_file = os.path.join(this_path, '..', '..', 'config', quad_name + '.xacro') # import from config, can use hummingbird for now to see if it matches 

    # Get parameters for drone
    attrib = parse_xacro_file(params_file) # hummingbird.xacro
    ...
    # borealis - x configuration
    elif quad_name == "borealis":
        # quad.max_thrust = float(attrib["max_rot_velocity"]) ** 2 * float(attrib["motor_constant"])
        # quad.max_thrust = float(1)
        quad.length_x = float(attrib['arm_length_front_x'])
        quad.length_y = float(attrib['arm_length_front_y'])
        hx = np.cos(np.pi / 4) * quad.length_x
        hy = np.cos(np.pi / 4) * quad.length_y
        quad.x_f = np.array([hx, -hx, -hx, hx])
        quad.y_f = np.array([-hy, -hy, hy, hy])
        quad.z_l_tau = np.array([-quad.c, quad.c, -quad.c, quad.c])
    ...
    return quad # returns the quad model from quadrotor 3d
...
class ROSGPMPC:
    def __init__(self, t_horizon, n_mpc_nodes, opt_dt, quad_name, point_reference=False, gp_models=None, rdrv=None):
        ...
        q_diagonal = np.array([0.3, 0.3, 0.3, 0.3, 0.3, 0.1, 0.05, 0.05, 0.05, 0.01, 0.01, 0.01]) # state weights XYZ = 0.5 for aggressive, 0.3 for gentler flight
        r_diagonal = np.array([5.0, 5.0, 5.0, 5.0]) # control inputs weights ALL = 1.0 for usual, 0.7 for AIM, supposely more agile 
    ...
    def set_state(self, x): # define state variables with att in quart
        """
    ...
    def optimize(self, model_data, enable_gp=False, gp_reg_ensemble=None):

        w_opt, x_opt = self.quad_mpc.optimize(use_model=model_data, return_x=True, enable_gp=enable_gp, gp_reg_ensemble=gp_reg_ensemble) # thrusters n body rates 
        ...
        #next_control.collective_thrust = np.sum(w_opt[:4]) * self.quad.max_thrust * self.quad.motor_coeff_downsize / (self.quad.mass * self.quad.g[2])
        next_control.collective_thrust = np.sum(w_opt[:4]) * (self.quad.max_thrust / self.quad.mass) * self.quad.motor_coeff_downsize
        # next_control.collective_thrust = np.sum(w_opt[:4]) 
        # no.of rows inside x_opt is self.N + 1 with 0 being the initial state given 
        next_control.velocity.x = x_opt[2, -6] # can use 1 for feedforward velocity and bod rates
        next_control.velocity.y = x_opt[2, -5]
        next_control.velocity.z = x_opt[2, -4]
        next_control.bodyrates.x = x_opt[2, -3]
        next_control.bodyrates.y = x_opt[2, -2]
        next_control.bodyrates.z = x_opt[2, -1]
