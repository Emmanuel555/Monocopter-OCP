...
for a in range(len(wing)): # 2 trajs
    ...
    for i in range(2): # 2 methods INDI vs NDI
        ...
        # motor
        start = wing[a][i][11][0]
        traj_time_motor_3 = wing[a][i][11] - start
        motor_cmd_3 = wing[a][i][13]   
        
        graphs[a].plot(traj_time_motor_3, motor_cmd_3, color=colors[i], linestyle='solid', linewidth=3.0, label=method_label) 
