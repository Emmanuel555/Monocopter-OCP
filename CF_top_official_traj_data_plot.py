...
file_path = '/home/emmanuel/Monocopter-OCP/circle/'
files = os.listdir(file_path)
files.sort(key=lambda x: os.path.getmtime(os.path.join(file_path, x)), reverse=True)

last_file = files[0]
circle_data = loadmat(os.path.join(file_path, last_file)) ##

file_path = '/home/emmanuel/Monocopter-OCP/elevated_circle/'
files = os.listdir(file_path)
files.sort(key=lambda x: os.path.getmtime(os.path.join(file_path, x)), reverse=True)

last_file = files[0]
elevated_circle_data = loadmat(os.path.join(file_path, last_file)) ##

file_path = '/home/emmanuel/Monocopter-OCP/lemniscate/'
files = os.listdir(file_path)
files.sort(key=lambda x: os.path.getmtime(os.path.join(file_path, x)), reverse=True)

last_file = files[0]
lem_data = loadmat(os.path.join(file_path, last_file)) ##


# circle
circle_time = circle_data['Data_time']
circle_position = circle_data['Monocopter_XYZ']
circle_ref_position = circle_data['ref_position']

px_circle = [row[0] for row in circle_position] # column vector
py_circle = [row[1] for row in circle_ref_position]
pz_circle = [row[2] for row in circle_ref_position]

px_circle = np.array([px_circle ])
py_circle = np.array([py_circle])
pz_circle = np.array([pz_circle])

px_circle_r = [row[0] for row in circle_ref_position] # column vector
py_circle_r = [row[1] for row in circle_ref_position]
pz_circle_r = [row[2] for row in circle_ref_position]

px_circle_r = np.array([px_circle_r])
py_circle_r = np.array([py_circle_r])
pz_circle_r = np.array([pz_circle_r])

# elevated circle
elevated_circle_time = elevated_circle_data['Data_time']
elevated_circle_position = elevated_circle_data['Monocopter_XYZ']
elevated_circle_ref_position = elevated_circle_data['ref_position']

# lemniscate
lem_time = lem_data['Data_time']
lem_position = lem_data['Monocopter_XYZ']
lem_ref_position = lem_data['ref_position']
