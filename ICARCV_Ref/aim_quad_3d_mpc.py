...
import numpy as np
#from src.quad_mpc.aim_quad_3d_optimizer import Quad3DOptimizer
from src.quad_mpc.exp_quad_3d_optimizer import Quad3DOptimizer
...
class Quad3DMPC:
    def __init__(self, my_quad, t_horizon=1.0, n_nodes=5, q_cost=None, r_cost=None,
        ...
        if pre_trained_models is not None:
            #self.gp_ensemble = restore_gp_regressors(pre_trained_models) # returns class GPEnsemble
            self.gp_ensemble = pre_trained_models
            ...
            #print ("size of B_x...", len(self.B_x))
            #print (x_dims)

        else:
            self.gp_ensemble = None # for now it would be none
        ...
        self.quad_opt = Quad3DOptimizer(my_quad, t_horizon=t_horizon, n_nodes=n_nodes,
                                        ...
                                        B_x=self.B_x, gp_regressors=self.gp_ensemble, # this is a gp ensemble
    ...
    def optimize(self, use_model=0, return_x=False, enable_gp=False, gp_reg_ensemble=None):
        """
        ...
        quad_current_state = self.quad.get_state(quaternion=True, stacked=True) # ding ding ding 
        ...
        out_out = self.quad_opt.run_optimization(quad_current_state, use_model=use_model, return_x=return_x,
                                                 gp_regression_state=None, enable_gp=enable_gp, gp_reg_ensemble=gp_reg_ensemble) # used to be quad_gp_state but theres nth there lol...
        return out_out
